package com.github.tianer2820.copperessentials.utils;
    ...
    public static Set<Block> getConnectedFloatingBlocks(
        ...
        while(!done){
            ...
            if(waveFront.size() + visited.size() > maxCapacity){
                CopperEssentials.getInstance().getLogger().warning(String.format("Connected log block exceeds max amount (%d), tree cut aborted.", maxCapacity));
